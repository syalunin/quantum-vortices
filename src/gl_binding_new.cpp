...
extern "C" void gl_set_colormap(CFI_cdesc_t* filename_) {
   if (textureCount >= 10) {
      std::cerr << "Reached the limit of textures!" << std::endl;
      std::exit(EXIT_FAILURE);
   }
   textureCount++
   ...
   glBindTexture(GL_TEXTURE_1D, texture[textureCount]);
...
// Draw an image
extern "C" void gl_draw_image(GLfloat* pos, GLsizei* shape,
   GLfloat* data1, GLfloat* data2, int textureIndex) {
